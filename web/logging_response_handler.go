...
func (self *loggingResponseWriter) log(err interface{}) {
	if err == nil {
		self.web.Infof("%v\t%v\t%v\t%v", self.request.Method, self.request.URL, self.status, time.Now().Sub(self.start))
	} else {
		self.web.Errorf("%v\t%v\t%v\t%v\t%v", self.request.Method, self.request.URL, self.status, time.Now().Sub(self.start), err)
	}
}

func (self *loggingResponseWriter) inc() {
	self.web.Infof("%v\t%v", self.request.Method, self.request.URL)
}
